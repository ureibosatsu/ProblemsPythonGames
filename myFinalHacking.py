...
def display_password_list(window, location):
    ...
    pw = password_list[randint(0, 12)]
    return pw


def embed_password(password, size):
    # return a 20-character string,
    # with the password embedded in
    # random characters
    # -password is the str to pad
    # -size is the int number of chars in the padded str
    fill = '!@#$%^&*()-+=~[]{}'
    embedding = ''
...
# display a hint on the right side of the window
def display_hint(window, password, guess, location):
    first_line = '{} INCORRECT'.format(guess)
    display_line(window, first_line, location)
    correct = 0
    index = 0
    maxlen = len(password)
    for letter in guess:  # check letters
        if (index < maxlen) and letter == password[index]:
            correct += 1
        index += 1
    correct = str(correct)
    second_line = '{}/7 IN MATCHING POSITIONS'.format(correct)
    display_line(window, second_line, location)
